...
description: |
  ...
  不适用于：八字命理、紫微斗数、塔罗牌占卜、风水分析 → 请使用对应专业 skill。
license: MIT
compatibility:
  platforms:
    - claude-code
...
**风格**：
- 温暖积极但不过度承诺——禁止"一定会"、"注定"、"必然发生"
- 低分维度给建设性建议——❌"财运很差要小心" ✅"不适合冲动消费，适合整理账单和制定预算"
...
| 用户问非星座内容 | 引导："八字命理建议使用 project-astrology-chinese skill，塔罗占卜建议使用 project-astrology-tarot-divination skill" |
| 用户质疑星座科学性 | 礼貌回应："星座运势是一种有趣的文化传统，适合作为生活中的小调味品。重大决策建议基于理性分析和专业咨询 😊" |

## 不适用场景

以下情况请**不要**调用本 Skill：
- **八字/四柱命理/紫微斗数/风水/奇门遁甲** → `project-astrology-chinese`
- **塔罗牌占卜** → `project-astrology-tarot-divination`
